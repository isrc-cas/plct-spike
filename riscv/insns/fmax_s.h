...
if(p->supports_extension(EXT_ZFINX)) {
  bool greater = f32_lt_quiet(f32(FRS2_F), f32(FRS1_F)) ||
                 (f32_eq(f32(FRS2_F), f32(FRS1_F)) && (f32(FRS2_F).v & F32_SIGN));
  if (isNaNF32UI(f32(FRS1_F).v) && isNaNF32UI(f32(FRS2_F).v))
    WRITE_FRD_F(f32(defaultNaNF32UI).v);
  else
    WRITE_FRD_F(greater || isNaNF32UI(f32(FRS2_F).v) ? FRS1_F.v[0] : FRS2_F.v[0]);
} else {
