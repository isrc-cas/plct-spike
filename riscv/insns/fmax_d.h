...
if(p->supports_extension(EXT_ZFINX)) {
  bool greater = f64_lt_quiet(f64(FRS2_D), f64(FRS1_D)) ||
                (f64_eq(f64(FRS2_D), f64(FRS1_D)) && (f64(FRS2_D).v & F64_SIGN));
  if (isNaNF64UI(f64(FRS1_D).v) && isNaNF64UI(f64(FRS2_D).v))
    WRITE_FRD_D(f64(defaultNaNF64UI).v);
  else
    WRITE_FRD_D(greater || isNaNF64UI(f64(FRS2_D).v) ? FRS1_D.v[0] : FRS2_D.v[0]);
} else {
